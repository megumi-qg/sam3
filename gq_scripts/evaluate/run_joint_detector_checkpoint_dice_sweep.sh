...
OUT_ROOT="${OUT_ROOT:-$EXP_DIR/dice_sweep_val}"
...
declare -A DATASET_DIRS=(
  [acdc]="/home/gaoqi/dataset/using/acdc/processed/sam3_png_coco_fullframes_100/val"
  [mscmr]="/home/gaoqi/dataset/using/mscmr/processed/png_coco_sam3_fullframes/val"
  [isbi]="/home/gaoqi/dataset/using/isbi/processed/png_coco_sam3_fullframes_train_val_test/val"
)
...
for ckpt_name in "${CKPTS[@]}"; do
  ...
  for dataset in acdc mscmr isbi; do
    ...
    python gq_scripts/evaluate/batch_inference.py \
      --test_dir "$test_dir" \
      --annotation_file full_annotations.coco.json \
