...
def merge_lora_into_sam3(model: nn.Module) -> List[str]:
    """
    将 LoRA 的 delta_W = (lora_alpha/r) * B @ A 合并回原始 W，
    并把 LoRALinear 替换为普通 nn.Linear，便于推理时用标准模型加载。

    forward 中 LoRA 项为 (x @ A @ B) * scaling，即 x @ (B.T @ A.T).T * scaling，
    nn.Linear 为 x @ W.T，故 delta_W = (B.T @ A.T) * scaling，形状 (out_features, in_features)。
    W_merged = W + delta_W。
