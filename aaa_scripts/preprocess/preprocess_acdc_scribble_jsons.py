...
用法示例：
 python scripts/preprocess_acdc_scribble_jsons.py \
...
def process_split(dataset_root: Path, output_root: Path, split: str, slice_policy: str = 'nonempty'):
    ...
    scribble_dir = dataset_root / split_dir_name / "scribble_bench"

    if not img_dir.exists() or not scribble_dir.exists():
        logging.warning(f"Preprocessed directories not found: {img_dir} or {scribble_dir}, skipping {split}.")
        return
    ...
    for img_path, scribble_path in tqdm(file_pairs, desc=f'Volumes ({split})'):
        ...
        for z in range(depth):
            ...
            # 保存可视化 Mask (可选，仅用于肉眼检查，将 4 变 0 方便显示)
            if not save_scribble_mask.exists():
                scribble_mask_to_save = scribble_slice.copy()
                scribble_mask_to_save[scribble_mask_to_save == IGNORE_LABEL] = 0
                Image.fromarray(scribble_mask_to_save.astype(np.uint8), mode='L').save(save_scribble_mask)
